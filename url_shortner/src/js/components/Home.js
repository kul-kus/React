...
import { NavigationBar, Footer, InputPanel, TextBox, ConvertStringtoHtml,LoadingBar } from "./commonComponent"

//--------thunk inports---------
import { useSelector, useDispatch } from "react-redux"
import { setSubmitAct, setAllValue, resetAllValue } from "./../actions"
  ...
  return (
    <>
      <Row style={{ "marginBottom": "10px" }}>
        <Col lg={{ span: 6, offset: 1 }} style={{ "textAlign": "right", "paddingTop": "6px" }}>
          {props.label}
        </Col>
...
function MyForm(props) {
  ...
  const { longUrl, shortUrl, isSumitted } = useSelector((state) => state.esy)
  ...
  const onFormSubmit = (e => {
    dispatch(setSubmitAct(true))

    e.preventDefault()
    validationCheck(formData, (err, data) => {
      if (err) {
        dispatch(setSubmitAct(false))
        setError(err)
      } else {
  ...
  let renderData = (
    <>
      ...
      <div className="error" style={errorCSS}>{formData.error}</div>
  ...
  return (
    <>
      {
        (isSumitted && longUrl && shortUrl) ? <><Redirect to="/shorturl"
        /></> : renderData
      }
    </>
...
const Home = () => {
  ...
  return (
    <>
      ...
      <LoadingBar type="home"></LoadingBar>
